...
add_task(async function test_stale_answer_revalidated_and_succeeds() {
  const host = "optimistic-dns.example.com";
  ...
  // the Happy Eyeballs path (so the cache entry is keyed exactly as the real
  // request below). The connection is refused, but the A record is cached with
  // the 1s TTL set above.
